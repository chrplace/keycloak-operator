...
func Test_KeycloakDeployment_With_Overrided_Image(t *testing.T) {
	...
	// when
	for _, ele := range KeycloakDeployment(cr).Spec.Template.Spec.Containers {
...
func Test_KeycloakDeploymentReconciled_With_Overrided_Image(t *testing.T) {
	...
	// when
	currentState := KeycloakDeployment(cr)
	...
	reconciledState := KeycloakDeploymentReconciled(cr2, currentState)
