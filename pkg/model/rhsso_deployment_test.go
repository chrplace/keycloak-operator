...
func Test_RHSSODeployment_With_Overrided_Image(t *testing.T) {
	...
	// when
	for _, ele := range RHSSODeployment(cr).Spec.Template.Spec.Containers {
...
func Test_RHSSODeploymentReconciled_With_Overrided_Image(t *testing.T) {
	...
	// when
	currentState := RHSSODeployment(cr)
	...
	reconciledState := RHSSODeploymentReconciled(cr2, currentState)
